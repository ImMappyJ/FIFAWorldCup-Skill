...
**每场比赛一个独立 HTML。内容:**
Header(对阵标题) → Group context strip → Weather + Travel badge → Win probability bars → 4-script narratives → Score heatmap → Goal cards → 半全场 9-grid → Full odds table(5 markets) → 导航链接
...
所有 HTML 转为 PNG 存入 `images/` 文件夹，与 HTML 同名。750px × 2× DPI, fullPage:true, 暗色背景 `#060D1A`。

Scripts: [screenshot.ps1](scripts/screenshot.ps1) (Edge, --window-size=750,8000) | [export.js](scripts/export.js) (Puppeteer, fullPage:true).
